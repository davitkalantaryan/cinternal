...
CPPUTILS_UTEST_AFTER_MAIN(f_0004_threading, t_0001) {
    ...
    CinternalUnitTestAssertCheck(cinternal_thread_create(&aThread, &ThreadFunction,CPPUTILS_NULL)==0);
    ...
    
    {
        ...
        cinternal_thread_detach(&aThread);
        CinternalUnitTestAssertCheck(cinternal_thread_attach(&aThread, dwId)==0);
